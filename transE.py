...
class TransEModel(object):
    ...
    def __init__(self, config):
        ...
        if config.L1_flag:  # use L1-norm
            self.fp = tf.norm((pos_h_e+pos_r_e-pos_t_e),ord=1,axis=1)
            self.fn = tf.norm((neg_h_e+neg_r_e-neg_t_e),ord=1,axis=1)

        else:  # use L2-norm
            self.fp = tf.norm((pos_h_e+pos_r_e-pos_t_e),ord=2,axis=1)
            self.fn = tf.norm((neg_h_e+neg_r_e-neg_t_e),ord=2,axis=1)
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        config_gpu.gpu_options.per_process_gpu_memory_fraction = 0.65
