...
class TransEModel(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("embedding"):
            ...
            self.M_embeddings = tf.get_variable(name="M", shape=[relation_total,size,size],initializer=tf.contrib.layers.xavier_initializer(uniform=False)) 
            pos_M_r = tf.nn.embedding_lookup(self.M_embeddings, self.pos_r)
            pos_h_e = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_h)
            pos_t_e = tf.nn.embedding_lookup(self.ent_embeddings, self.pos_t)
            pos_r_e = tf.nn.embedding_lookup(self.rel_embeddings, self.pos_r)
            neg_M_r = tf.nn.embedding_lookup(self.M_embeddings, self.neg_r)
            ...
            hrp = tf.einsum("ni,nik->nk",pos_h_e,pos_M_r)
            trp = tf.einsum("ni,nik->nk",pos_t_e,pos_M_r)
            hrn = tf.einsum("ni,nik->nk",neg_h_e,neg_M_r)
            trn = tf.einsum("ni,nik->nk",neg_t_e,neg_M_r)                
                            
        if config.L1_flag:  # use L1-norm
            self.fp = tf.norm((hrp+pos_r_e-trp),ord=1,axis=1)
            self.fn = tf.norm((hrn+neg_r_e-trn),ord=1,axis=1)

        else:  # use L2-norm
            self.fp = tf.square(tf.norm((hrp+pos_r_e-trp),ord=2,axis=1))
            self.fn = tf.square(tf.norm((hrn+neg_r_e-trn),ord=2,axis=1))
           
        with tf.name_scope("output"):
            self.loss = tf.reduce_sum(tf.nn.relu(margin+self.fp-self.fn))
